...
async def set_mod_command(app: Client, message: Message):
    ...
    if user and 'is_mod' in user and user['is_mod'] is True:
        await message.reply_text(text="Este usuario ya es moderador.")
        return

    result = await add_moderator(reply_user_id)
...
async def del_mod_command(app: Client, message: Message):
    ...
    if not user or ('is_mod' in user and user['is_mod'] is False):
        await message.reply_text(text="Este usuario no es moderador.")
        return

    result = await remove_moderator(reply_user_id)
    await message.reply_text(text=result)
